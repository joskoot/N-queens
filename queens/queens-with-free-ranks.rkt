...
(define (queens N (show-sols? #t) (whole-classes? #t) (show-boards? #t))
 ...
 ; Keeping record of all attacked fields is not faster and requires much more memory.

 (define pure-sol car)
 (define free-ranks-of-sol cdr)
 (define make-sol cons)
 (define set-of-all-ranks (if (zero? N) 0 (make-natset (list 0 N))))
             ...
             (loop2 (add1 rank)
              (make-sol
               (cons (cons rank (pure-sol old-part-sol))
                (natset-subtract (free-ranks-of-sol old-part-sol) (make-natset rank)))
               new-part-sols)))
            (else (loop2 (add1 rank) new-part-sols)))))))))))))

 (define (safe? part-sol new-rank new-file)
  (and
   (natset-member? (free-ranks-of-sol part-sol) new-rank)
 ...
 (define (attacks? file-a rank-a file-b rank-b)
  (or
 ;  (= rank-a rank-b) ; already accounted for by (free-ranks-of-sol part-sol)
   (= file-a file-b)
...
(for ((N (in-range 0 15))) (time (queens N #f #f #f)))
