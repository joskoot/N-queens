#lang racket/base
 ...
 as a list (rank[0] .. rank[N-1]) or as (file[0] .. file[N-1]) where rank[i] indicates which rank of
 file i contains its queen and file[i] indicates in which file rank i has its queen. Solutions are
 found by placing queens in non attacked ranks of successive files. (rank[0] .. rank[K-1]) with K<N-1
 is a partial solution. Given a list of partial solutions, a new list of partial solutions is made
 with one file more. For each old partial solution every rank of the next file is tried. In the
 program new ranks are consed to partial solutions, which means that reversed solutions are found, but
 this is irrelevant, for the list of full solutions includes the reverse of each solution, reversal
 corresponding to reflection in the vertical center line (assuming files to be vertical). For N<4 the
 solutions are returned immediately. For N≥4 we have the following nested loops:
 
 1. generate-new-solutions: Start with all partial solutions of one file: ((0) (1) .. (N-1)). In every
    cycle the list of partial solutions of f files is replaced by a new list of partial solutions of
    f+1 files. When f reaches N, the partial solutions are the full solutions.
 ...
 files and therefore can be rejected immediately. This is done in generate-new-solutions-2, which is
 used when argument look-ahead is true. This can reduce the number of cycles in the partial-solution-
 loop. However, I guess that checking for each partial solution that there is a free rank in all files
 yet to come does not increase speed. I checked for N=15 and found the maximum number of computed
 partial solutions to be halved but execution time doubled. 
 ...
 (define (queens N (show-solutions? #t) (whole-classes? #t) (show-boards? #t) (look-ahead? #f))
  ...
  (define (generate-solutions)
   (case N
    ...
    ((2 3) '())
    (else
     ; Start with all N partial solutions of one file (file 0), id est ((0) (1) (2) (N-1)).
     ((if look-ahead? generate-new-solutions-2 generate-new-solutions) 1 (build-list N list)))))
 
  (define (generate-new-solutions file old-partial-solutions)
   (cond
    ...
    (else
     (let partial-solution-loop
      ...
      (cond
       ...
       ((null? old-partial-solutions) (generate-new-solutions (add1 file) new-partial-solutions))
  ...
  (define (generate-new-solutions-2 file old-partial-solutions)
   (cond
    ; File loop. With file=N we have all full solutions and we can return them.
    ((= file N) old-partial-solutions)
    ; Otherwise we have to find all partial solutions with one more file.
    ...
    (else
     (let partial-solution-loop
      ...
      (cond
       ; After all old partial solutions have been done, return to the file-loop.
       ((null? old-partial-solutions) (generate-new-solutions-2 (add1 file) new-partial-solutions))
       ; Otherwise find new partial solutions extending the old partial solution with one file.
       (else
