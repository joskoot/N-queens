...
Find all solutions of placing N queens on a NxN chess board, such that no queen attacks another queen.
The rows of a chess board are called 'ranks', the columns are called 'files'. A chess player numbers
ranks from 1 up to and including the number of ranks. The files are identified by the letters 'a',
'b', 'c', etc. In the present program both files and ranks are identified by numbers 0 up to but not
including N. A solution has exactly one queen in every file and in every rank. Therefore a solution
can be written as a list (rank[N-1] .. rank[0]) where rank[i] indicates which rank of file i contains
its queen. Solutions are made by placing queens in non attacked ranks of successive files.
(rank[K] .. rank[0]) with K<N-1 is a partial solution. Given a list of partial solutions, a new list
of partial solutions is made with one file more. For each old partial solution every rank of the next
file is tried. For N<4 the solutions are returned immediately. For N≥4 we have the following nested
loops:

1. file-loop: Start with all partial solutions of one file: ((0) (1) .. (N-1)). In every cycle the
   list of partial solutions of f files is replaced by the list of partial solutions of f+1 files.
...
4. safe? and attack?: Within rank-loop the rank must be tested not being attacked. This involves a
   short loop cycling over the length the old partial solution.

Loop 2 may have many cycles. The other loops have N or less cycles. After all solutions have been
found, the list of solutions is divided in sublists of symmetrically equivalent solutions. A sublist
of symmetrically equivalent solutions is an equivalence-class.
...
1. By placing queens only in the lower half of the first file and after all solutions have been found
   adding the reflections of all solutions in the horizontal center line (regarding ranks horizontal,
   and files vertical).
...
For each partial solution keeping record of attacked fields in files yet to be added does not
increase speed and would require much more memory.
...
(define (queens N (show-solutions? #t) (whole-classes? #t) (show-boards? #t))
 ...
 (define (generate-solutions)
  (case N
   ...
   (else
    ...
    (let file-loop ((file 1) (old-partial-solutions (build-list N list)))
    (cond
     ; With file=N we have all full solutions and we can exit procedure generate-solutions.
     ...
     (else
      (let partial-solution-loop
       ...
       (cond
        ...
        ((null? old-partial-solutions) (file-loop (add1 file) new-partial-solutions))
 ...
 (define (make-classes)
  ; Take all symmetrical solutions of a solution. They form a class. Do this for every solution.
  ; Variable done contains a set of all solutions already in a registered class.
  ; It protects against duplicate classes.
  (define setequal set)
  (define classes
   ...
   ; not necessarily eqv? (let alone eq?) to one of the list of all solutions. This also avoids the
   ; computations of symmetrical equivalent solutions of solutions already registered in a class.
 ...
 (define (print-board solution)
  ...
  (for ((n range-N))
   (display
    (overlay
     (text/font
      (format "~s" (vector-ref rank-vector n))
      17
      'black
      #f
      'roman
